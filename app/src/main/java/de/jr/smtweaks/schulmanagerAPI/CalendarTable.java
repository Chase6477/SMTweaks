...
public class CalendarTable {
    ...
    private static Calendar getMonday() {
    ...
    public static void fetchData(String token, String student, OnFinishedFetching listener) {
        Calendar sunday = getMonday();
        sunday.add(Calendar.DAY_OF_WEEK, 7);
        RequestBody body = RequestBody.create("{\"bundleVersion\":\"" + Login.BUNDLE_VERSION + "\",\"requests\":[{\"moduleName\":\"schedules\",\"endpointName\":\"get-actual-lessons\",\"parameters\":{\"student\":" + student + ",\"start\":\"" + formatCalendar(getMonday()) + "\",\"end\":\"" + formatCalendar(sunday) + "\"}}]}", MediaType.get("application/json; charset=utf-8"));
